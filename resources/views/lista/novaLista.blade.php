...
<div class="row">
	<div class="col-md-8 col-md-offset-2">
		<form method="POST" action={{route('novaLista')}}>
			<input type="hidden" name="_token" value="{{csrf_token()}}" />
			<div class="form-group{{ $errors->has('nomeLista') ? ' has-error' : '' }}">
				<label for="nomeLista">Nome (título) da lista:</label>
				<input required id="nomeLista" name="nomeLista" type="text"  value="{{ old('nomeLista') }}" class="form-control" autofocus>
			...
			<div class="form-group{{ $errors->has('descricaoLista') ? ' has-error' : '' }}">
				...
				<textarea required  id="descricaoLista" name="descricaoLista" type="text" rows="3" class="form-control">{{ old('descricaoLista') }}</textarea>
